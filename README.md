...
在 [XDGAME](https://www.xdgame.com/) 游戏列表和更新记录中直接显示小黑盒评分、Steam 全语言好评率、评价人数和当前在线人数的 Tampermonkey 用户脚本。
...
- 支持游戏列表和[更新记录/天梯排名](https://www.xdgame.com/update/)页面，包括切换后的各个排行榜标签。
- 更新记录页面采用固定宽度评分区，长游戏名自动省略并保留悬停完整标题，使每一行保持对齐。
- 缩略图没有 Steam AppID 时，会从 XDGAME 游戏详情页补充识别并在本地缓存结果。
...
4. 打开或刷新 [XDGAME 游戏列表](https://www.xdgame.com/list/1/)或[更新记录](https://www.xdgame.com/update/)页面。
...
- XDGAME 游戏列表、更新记录/天梯排名页面及包含相同游戏卡片结构的页面
...
- Supports the game list and the [Updates/Ranking](https://www.xdgame.com/update/) page, including every ranking tab after it is selected.
- Uses a fixed-width rating area on the Updates page. Long titles are truncated consistently, while the complete title remains available on hover.
- Falls back to the XDGAME game detail page when a thumbnail does not contain a Steam AppID and caches the resolved ID locally.
...
4. Open or refresh the [XDGAME game list](https://www.xdgame.com/list/1/) or [Updates/Ranking](https://www.xdgame.com/update/) page.
...
- XDGAME game lists, the Updates/Ranking page, and pages using the same game-card structure
...
- ゲーム一覧と[更新履歴・ランキング](https://www.xdgame.com/update/)ページに対応し、切り替え後の各ランキングタブでも動作します。
- 更新履歴ページでは評価欄の幅を固定し、長いゲーム名を省略表示して各行を揃えます。完全なタイトルはマウスを重ねると確認できます。
- サムネイルに Steam AppID がない場合は XDGAME のゲーム詳細ページから補完し、結果をローカルにキャッシュします。
...
4. [XDGAME ゲーム一覧](https://www.xdgame.com/list/1/)または[更新履歴・ランキング](https://www.xdgame.com/update/)ページを開くか、再読み込みします。
...
- XDGAME のゲーム一覧、更新履歴・ランキングページ、および同じゲームカード構造を使用するページ
...
- 新增 XDGAME 更新记录/天梯排名页面支持。
- Added support for the XDGAME Updates/Ranking page.
- XDGAME の更新履歴・ランキングページに対応しました。
- 统一横向列表的标题、评分区和日期宽度，并加入移动端布局。
- Added aligned desktop rows, title truncation, and a responsive mobile layout.
- 横並びリストの整列、タイトル省略、モバイル表示を改善しました。

### v2.0.0

- 新增小黑盒风格评分卡、全语言好评率、心愿单回退和当前在线人数。
- Added Xiaoheihe-style cards, all-language ratings, wishlist fallback, and online player counts.
- 小黒盒風カード、全言語好評率、ウィッシュリスト表示、オンライン人数を追加しました。
