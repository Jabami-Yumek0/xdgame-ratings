...
// @version      2.0.0
// @description  在 XDGAME 游戏卡片中显示小黑盒评分、全语言好评率与当前在线人数。
// @author       LIU
// @match        *://xdgame.com/*
// @match        *://*.xdgame.com/*
...
(function () {
  ...
  function scanCards() {
    for (const card of document.querySelectorAll(".game-list > li")) {
      const appId = findSteamAppId(card);
      if (!appId) {
        renderMissingId(card);
        continue;
      }

      if (card.dataset.xdgrLoaded === appId) {
        continue;
      }

      card.dataset.xdgrLoaded = appId;
      const container = ensureContainer(card, appId);
  ...
  function ensureContainer(card, appId) {
    ...
    if (!container) {
      ...
      const data = card.querySelector(":scope > .data");
      card.insertBefore(container, data || null);
    }
    return container;
  }
